...
class Waveform(customtkinter.CTkFrame):
    ...
    def draw_waveform(self, *args):
        ...
        for i in indices:
            ...
            if self.mode == "in":
                ...
                coords = [(x, h, x+80, h), (y, h, y-80, h)]
                self.canvas.create_rectangle(y, 0, y + 5, self.canvas_height, fill=self.removed_fill,
                                         outline=self.removed_fill, tags=("removed", str(i)))
                self.canvas.create_line(coords[0], fill=self.removed_fill, tags=("removed", str(i)),
                                        width=5, arrow=tk.LAST)
                self.canvas.create_line(coords[1], fill=self.removed_fill, tags=("removed", str(i)),
                                        width=5, arrow=tk.LAST)
    ...
    def selected_speech_event(self, event):
        ...
        if tags.split(" ")[0] == "removed":
            click = self.other_frame.signal_mismatch[int(id)]
            if self.mode == "out":
                click = click[0]
